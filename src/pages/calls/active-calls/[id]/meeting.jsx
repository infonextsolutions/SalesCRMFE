...
const MeetingProfile = () => {
  ...
  const checkCallStatus = (callRes) => {
    ...
    axios
      .post(`api/calling/call-status`, payload, {
        headers: { Authorization: accessToken },
      })
      .then((res) => {})
      .catch((err) => {
...
//             `${baseUrl}api/meeting/find-by-id?id=${params.id}`
//         );
//         // const another = await axios.get(
//         //   `${baseUrl}api/meeting-script/active-call?activeCallId=${response?.data?.result?._id}`
//         // );
//         return {
//             props: {
