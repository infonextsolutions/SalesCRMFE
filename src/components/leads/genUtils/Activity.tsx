...
const MidPath = ({active}:any) => {
  return (
    <div className="w-[31px] flex items-center justify-between">
      <div  className={`w-[4px] h-[4px] rounded-[50%] ${active ? "bg-[#B656EB]" : "bg-gray-400"}`}></div>
      <div className={`w-[19px] h-[4px] rounded-[12px] ${active ? "bg-[#B656EB]" : "bg-gray-400"}`}></div>
      <div className={`w-[4px] h-[4px] rounded-[50%] ${active ? "bg-[#B656EB]" : "bg-gray-400"}`}></div>
    </div>
  );
};

const SingleActivity = ({act,i,last}:any) =>{
  ...
  return (
          ...
          setHover(false);
            ...
            <Image src={getBasicIcon(act.activityType)} alt="" width={15} height={15} />
            {last > i && <MidPath active={true}/>}
            {hover && (
          <div
...
const ActivityHistory = ({ width, left, last }: any) => {
  ...
  return (
    <>
      <div
        className={`flex items-center justify-center h-[20px] relative shrink-0 cursor-pointer`}
      ...
      {/* {
          Activity.map((act,i)=>
          {
            return  <SingleActivity act={act} key={i}/>
          }
        } */}

        {
          Activity.map((act,index)=>{
            return (
              last>=index ?
              <SingleActivity act={act} last={last} i={index} key={index}/> :(
                <div className="flex justify-between">
                  {Activity.length > index && <MidPath active={false}/>}
                  <Image src={getBasicIcon("minus")} alt="" width={15} height={15} />
                </div>
              )
            )
          })
