...
const Metting_LeadsTable = ({ totalRecords, search, queryStr }: TableProps) => {
  ...
  useEffect(function () {
    axios.get(
      `https://sales365.trainright.fit/api/event/find-all?limit=${limit}&page=${pageNumber}${queryStr}`, {
      headers: {
  ...
  const fetchItems = async (current: any) => {
    const res = await axios.get(
      `https://sales365.trainright.fit/api/event/find-all?limit=${limit}&page=${current}`, {
      headers: {
        Authorization: accessToken
      }
    }
