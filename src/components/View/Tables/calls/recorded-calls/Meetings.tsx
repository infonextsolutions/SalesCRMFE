...
const LeadsTable = ({ totalRecords, search, queryStr }: TableProps) => {
  ...
  useEffect(function () {
    axios.get(
      `https://salescrmbe.onrender.com/api/meeting/find-all?limit=${limit}&page=${pageNumber}${queryStr}`
    ).then(res => {
  ...
  const getallItems = async (current: any) => {
    const res = await axios.get(
      `https://salescrmbe.onrender.com/api/meeting/find-all?limit=${limit}&page=${current}${queryStr}"`
    );
  ...
  useEffect(() => {
    if (checked) {
      ...
      const getItems = async () => {
        const res = await axios.get(
          `https://salescrmbe.onrender.com/api/meeting/find-all?${queryStr}`
        );
  ...
  const fetchItems = async (current: any) => {
    const res = await axios.get(
      `https://salescrmbe.onrender.com/api/meeting/find-all?limit=${limit}&page=${current}`
    );
    const data = res.data.result;
