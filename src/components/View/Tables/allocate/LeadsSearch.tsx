...
const LeadsTable = ({ totalRecords, search, queryStr }: TableProps) => {
  ...
  useEffect(
    function () {
      axios
        .get(
          `https://sales365.trainright.fit/api/leads/find-all?id=${qaid}&limit=${limit}&page=${pageNumber}${queryStr}`, {
  ...
  const getallItems = async (current: any) => {
    try {
      const res = await axios.get(
        `https://sales365.trainright.fit/api/leads/find-all?id=${qaid}&limit=${limit}&page=${current}${queryStr}`, {
        headers: {
  ...
  useEffect(() => {
    try {
      ...
      const getItems = async () => {
        const res = await axios.get(
          `https://sales365.trainright.fit/api/leads/find-all?id=${qaid}${queryStr}`, {
          headers: {
            Authorization: accessToken
          }
        }
  ...
  const fetchItems = async (current: any) => {
    try {
      const res = await axios.get(
        `https://sales365.trainright.fit/api/leads/find-all?id=${qaid}&limit=${limit}&page=${current}`, {
        headers: {
          Authorization: accessToken
        }
      }
